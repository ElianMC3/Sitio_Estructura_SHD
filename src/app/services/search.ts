...
//los services tienenfunciones especificas
export type SearchFilters = {
    type?: 'todos' | 'pagina' | 'seccion';
    section?: 'todas' |string;
};
//el operador spread (...) permite copiar todas las propiedades de un objeto a otro objeto, hace un clon para no modificar el original
  ...
  search(query: string, filters: SearchFilters): Observable<SiteItem[]> {
    ...
    if (q.length > 0) {
      ...
      data = data.filter(item => {
        ...
        const haystack =
          (item.tittle + ' ' + item.description + ' ' + item.keywords.join(' '))
            .toLowerCase();
